...
impl Camera {
    ...
    pub fn project_vertex_into(
        ...
        if clip_pos.w <= 0.0 || !self.is_in_clip_space(&clip_pos) {
            out.pos.x = -1.0;
            out.pos.y = -1.0;
    ...
    fn is_in_clip_space(&self, clip_pos: &glam::Vec4) -> bool {
        let w = clip_pos.w;

        let x: bool = clip_pos.x >= -w && clip_pos.x <= w;
        let y: bool = clip_pos.y >= -w && clip_pos.y <= w;
        let z: bool = clip_pos.z >= -w && clip_pos.z <= w;
